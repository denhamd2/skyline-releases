...
repository, not from the private source repository.

The app sends no credentials, so it cannot read the private repository's
release API or download its assets. CI mirrors every APK to the public repo
for this reason.
