...
### Requirement: APKs are mirrored to a public repository

CI SHALL publish each APK to both the private source repo and the public
`denhamd2/skyline-releases`, under the rolling `skyline-latest` tag.

#### Scenario: Publishing a build

- **WHEN** a build succeeds on the development branch
- **THEN** `Skyline.apk` is uploaded to both repositories

#### Scenario: Publishing credential is absent

- **WHEN** the `RELEASES_TOKEN` secret is not configured
- **THEN** the mirror step is skipped and the build still succeeds
